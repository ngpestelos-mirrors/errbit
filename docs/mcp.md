...
- `errbit_list_apps`: lists applications ordered by name. It defaults to 25
  results and accepts `page` and `per_page`; `per_page` is capped at 100.
...
- `errbit_list_problems`: lists bounded problem summaries. It accepts optional
  `app_id`, `environment`, `resolved`, and text `search` filters, plus `page` and
  `per_page`; `per_page` is capped at 100. Results are ordered by latest notice
  time and then ID.
...
- `errbit_list_notices`: lists notice summaries for a problem, newest first. It
  accepts `page` and `per_page`; `per_page` is capped at 100. Use
