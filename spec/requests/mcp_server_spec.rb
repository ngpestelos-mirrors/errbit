...
RSpec.describe "MCP server", type: :request do
  ...
  it "lists the advertised read-only tools for a valid credential" do
    ...
    expect(response).to have_http_status(:ok)
    expect(json.dig("result", "tools").pluck("name")).to contain_exactly("errbit_list_apps", "errbit_get_app", "errbit_list_problems", "errbit_get_problem", "errbit_get_latest_notice", "errbit_list_notices")
  ...
  it "lists filtered, bounded problem metadata" do
    ...
    mcp_request(
      "tools/call",
      {
        ...
        arguments: {app_id: app.id, environment: "production", resolved: false, search: "NoMethodError", per_page: 1}
      },
      authorization: "Bearer #{token}"
    )
